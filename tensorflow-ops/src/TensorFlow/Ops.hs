...
instance ( TensorType a
         ...
         , OneOf '[ Double, Float, Int32, Int64
                  , Complex Float, Complex Double] a) => Num (TensorExpr a) where
    (+) = CoreOps.add
    (*) = CoreOps.mul
    (-) = CoreOps.sub
    abs = CoreOps.abs
...
initializedVariable :: forall a v . TensorType a
                    => Tensor v a -> Build (Tensor Ref a)
...
vector :: TensorType a => [a] -> TensorExpr a
