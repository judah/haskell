...
readVar :: TensorType a => Variable a -> Tensor Build a
readVar = readVar' id

readVar' :: forall a . TensorType a => OpParams -> Variable a -> Tensor Build a
...
restoreFromName :: forall a m . (MonadBuild m, TensorType a)
                ...
                -> m ControlNode
restoreFromName path name x@(Variable h) = build $ do
    path' <- buildInputs $ scalar path
    name' <- buildInputs $ scalar name
    restoreOp <- buildOp [] $ opDef "Restore"
                               & opAttr "dt" .~ tensorType (undefined :: a)
                               & opInputs .~ (path' ++ name')
    group =<< assign x (expr (restoreOp :: Tensor Value a))
...
restore :: forall a m . (MonadBuild m, TensorType a)
        ...
        -> Variable a  -- ^ Tensor to restore.
        -> m ControlNode
restore path x@(Variable h) = restoreFromName path name x
  where
    name = encodeUtf8 $ encodeOutput $ renderedOutput h
